...
DATA_FILE = "lich_su_gd.csv"

# --- HÀM QUẢN LÝ LỊCH SỬ GIAO DỊCH ---
def load_data():
    if not os.path.exists(DATA_FILE):
        df_init = pd.DataFrame(columns=["Ngày", "Mã CK", "Loại lệnh", "Giá (VNĐ)", "Khối lượng"])
...
with st.sidebar:
    ...
    with st.form("nhap_lenh_form", clear_on_submit=True):
        ...
        submit = st.form_submit_button("Lưu lệnh")
        if submit and ma_ck_moi and kl_nhap > 0:
            ...
            st.success(f"Đã lưu lệnh {loai_lenh} {ma_ck_moi}!")
            st.rerun()
...
# ==========================================
st.title("📈 Bảng điều khiển Danh mục & DCA")
